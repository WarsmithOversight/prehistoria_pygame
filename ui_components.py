...
from ui_dimensions import UI_ELEMENT_PADDING, BUTTON_CORNER_CHAOS_FACTOR, CARD_TEXT_PADDING
...
class UICard:
    ...
    """
    def __init__(self, size, center_pos, card_data, assets_state, event_bus):
        # ⚙️ Core Attributes
        ...
        # ✨ FIX: The text block's rect should be inset from the card's main rect by the padding amount.
        pad_w, pad_h = CARD_TEXT_PADDING
        
        # This rect defines the drawable area for the text, positioned correctly within the card's surface.
        # ✨ FIX: Correctly format the Rect constructor with position and size tuples.
        text_area_rect = pygame.Rect(
            (pad_w, pad_h),  # Top-left position as a tuple
            (size[0] - (pad_w * 2), size[1] - (pad_h * 2)) # Width and height as a tuple
        )
        ...
        self.text_block = UITextBlock(rect=text_area_rect, text=card_text, style=text_style, assets_state=self.assets_state)
    ...
    def draw(self, surface):
        ...
        # 🏞️ Blit Glow Effect (if active)
        # First, draw the pre-rendered procedural glow if the card is in a glowing state.
        if self.is_glowing and self.cached_glow_surface:
            # ✨ FIX: Calculate the correct position for the larger glow surface.
            # 1. Get a rect for the glow surface itself.
            # 2. Set its center to be the same as the card's rect center.
            # This automatically calculates the correct top-left (x, y) for the blit operation.
            glow_rect = self.cached_glow_surface.get_rect(center=self.rect.center)
            
            # 3. Blit the glow using the new, perfectly centered glow_rect.
            surface.blit(self.cached_glow_surface, glow_rect, special_flags=pygame.BLEND_RGBA_ADD)

        # ✍️ Blit Card Text
        # Draw the text block directly onto the target surface, providing our card's
        # top-left corner as the positional offset. This guarantees alignment.
        self.text_block.draw(surface, offset=self.rect.topleft)
